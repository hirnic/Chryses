...
class Defense:
    def __init__(self, name, buildCost, baseShields, baseDamage):
        self.name = name                                     # Name of Defense
        self.buildCost = buildCost                           # Metal, Crystal, Deuterium, Energy costs
        self.baseArmor = self.buildCost[0]+self.buildCost[1] # Armor rating of Defense
    ...
    def getTime(self, universeSpeed, roboticsFactory, naniteFactory): #in seconds
        timeNeeded = (self.buildCost[0] + self.buildCost[1])/(2500*(1+roboticsFactory)*universeSpeed*(2**naniteFactory))
        return max(1,int(3600*timeNeeded))

class Ship(Defense):
    def __init__(self, name, buildCost, baseDamage, baseShields, baseSpeed, baseCargo,
                baseFuel):
        Defense.__init__(self, name, buildCost, baseDamage, baseShields)
